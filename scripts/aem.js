...
function decorateTwoColumnSection(section) {
  if (!section.classList.contains('two-column')) return;

  const leftColumnItems = parseInt(section.dataset.leftColumnItems || '1', 10);
  const leftItemCount = Number.isNaN(leftColumnItems) ? 1 : Math.max(0, leftColumnItems);

  [...section.children].forEach((wrapper, index) => {
    wrapper.classList.toggle('two-column-left', index < leftItemCount);
    wrapper.classList.toggle('two-column-right', index >= leftItemCount);
  });
}
...
function decorateSections(main) {
  main.querySelectorAll(':scope > div:not([data-section-status])').forEach((section) => {
    ...
    decorateTwoColumnSection(section);
  });
}
